...
# NAS, copies the master .env + vault.key, and restarts the
# container — zero manual UI steps.
#
# Usage:
...
NAS_COMPOSE_DIR="/volume1/docker/vault"   # Synology path where compose lives
NAS_SMB_DIR="/mnt/k/vault"               # Fallback: Windows SMB mount
...
echo -e "${CYAN}${BOLD}  🔐 Vault — Deploy to Synology${RESET}"
if $DRY_RUN; then
...
# ── 3. Deploy ─────────────────────────────────────────────────
if $HAS_SSH; then
  # ── SSH path: pipe image + copy secrets + restart ──────────
  step "Deploying via SSH → ${NAS_HOST}"

  if $DRY_RUN; then
    info "(skipped — dry run)"
  else
    # Ensure compose + env directories exist on NAS
    ...
    # Copy master .env into the env/ mount directory
    info "Syncing master .env..."
    cat "$MASTER_ENV" | ssh "$NAS_HOST" "cat > '${NAS_COMPOSE_DIR}/env/.env'"
    ok "Master .env synced"

    # Copy vault.key into the env/ mount directory
    info "Syncing vault.key..."
    cat "$VAULT_KEY" | ssh "$NAS_HOST" "cat > '${NAS_COMPOSE_DIR}/env/vault.key'"
    ok "vault.key synced"
  ...
  if $DRY_RUN; then
    ...
    cp "$MASTER_ENV" "${NAS_SMB_DIR}/env/.env"
    cp "$VAULT_KEY" "${NAS_SMB_DIR}/env/vault.key"
    rm -f "/tmp/${TARBALL}"
    ...
    info "  2. Project → vault → Stop → Start"
  fi
fi
